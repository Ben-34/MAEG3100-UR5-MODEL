...
def parabolic_blend_coeffs(start, end, duration, t_b = None, acceleration = None):
    ...
    if t_b is not None:
        ...
        acceleration = (start - end) / ( t_b ** 2 - t_b * duration )
        ...
        t_b = (duration / 2) - ( np.sqrt( acceleration ** 2 * (duration **2) - 4 * acceleration * (end - start)) / (2 * np.abs(acceleration) ) )
...
def parabolic_blend_trajectory(start, end, t, duration, t_b = None, acceleration = None):
    ...
    for i in range(6):
        if(t < t_b[i]): # different joints can have different blending times t_b !
            target_pos[i] = (acceleration[i] / 2) * t**2 + start[i]
            target_speed[i] = acceleration[i] * t

        elif(t <= duration - t_b[i] ): 
            theta_b = acceleration[i] / 2 * t_b[i]**2 + start[i]

            target_pos[i] = acceleration[i]* t_b[i] * (t - t_b[i]) + theta_b #Theta_b == position of joint at time t_b !!!
            target_speed[i] = acceleration[i] * t_b[i]
        
        elif(t < duration):
            target_pos[i] = (-acceleration[i] / 2) * (t- duration)**2 + end[i] 
            target_speed[i] = acceleration[i] * (duration - t)
